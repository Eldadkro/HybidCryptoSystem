...
logger = build_logger('deciphering')
...
def calculate_modular_inverse(alg, n, m):
    ...
    """
    if alg == algorithm_brute_force:
        return modular_inverse_via_brute_force(n, m)
    elif alg == algorithm_totient:
...
def decipher_vector_elements(ciphered_vector, modulo, multiplicative):
    ...
    multiplicative_inverse = calculate_modular_inverse(algorithm_totient, multiplicative, modulo)
...
def deciphered_items_to_bit_sequence(alg, private_key_vector, deciphered_item):
    return solve_knapsack(alg, private_key_vector, deciphered_item, "")
