...
def read_property_key(key, structure_type, section, file_name):
    ...
    try:
        ...
        if structure_type == structure_type_int:
            return config.getint(section, key)
        elif structure_type == structure_type_float:
            return config.getfloat(section, key)
        elif structure_type == structure_type_boolean:
...
comma_delimiter = ","
hyphen_delimiter = "-"
log_enabled = read_property_key("log_enabled", structure_type_boolean, section_parameters,
                                property_file)
random_text_test = read_property_key("random_text_test", structure_type_boolean, section_parameters,
                                property_file)
length_of_random_text = 0
if random_text_test:
    length_of_random_text = read_property_key("length_of_random_text", structure_type_int,
                                              section_parameters, property_file)
length_of_key_vector = read_property_key("length_of_key_vector", structure_type_int,
                                         section_vector, property_file)
random_key_vector_addition_range = read_property_key("random_key_vector_addition_range", structure_type_int,
                                                     section_vector, property_file)
private_key_vector_initial_range = read_property_key("private_key_vector_initial_range", structure_type_string
                                                     , section_vector, property_file)
...
def validate_initial_parameters():
    ...
    """
    validation_message = ""
    log_enable_regex_check = check_regex_match(str(log_enabled), regex_pattern_boolean)
    initial_range_regex_check = check_regex_match(str(private_key_vector_initial_range), regex_pattern_range)
    random_key_addition_regex_check = check_regex_match(str(random_key_vector_addition_range), regex_pattern_digit)
    if not log_enable_regex_check:
        validation_message = "Log enabled parameter can set only true or false. " + \
                             "Rearrange it from " + str(property_file)
...
def user_input(m, r):
    ...
    while not check_regex_match(text_input, r):
        wrong_input = "\n" + background_colorant_red + "You entered a wrong input." + \
                      attribute_default + "\n" + m
...
def convert_text_to_bit(text, bit_length, encoding='utf-8', errors='surrogatepass'):
    ...
    for i in range(0, len(text)):
        char = text[i]
        #convert each byte into binary string
